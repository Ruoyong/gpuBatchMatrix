...
myRaster = terra::rast(
  terra::ext(0,0.5*Ngrid[2],5, 5 + 0.5*Ngrid[1]),
                        Ngrid[1], Ngrid[2])
coordsSp = terra::vect(terra::xyFromCell(
  myRaster, 1:terra::ncell(myRaster)))
coordsMat = terra::crds(coordsSp)
} else {
  coordsMat = cbind(
    rep(seq(from=0.25, by=0.5, len=Ngrid[2]), Ngrid[1]),
    rep(seq(to=5.25, by=-0.5, len=Ngrid[1]), each=Ngrid[2])
  )
}
...
```{r showfromcpu}
if(requireNamespace('geostatsp', quietly=TRUE) & exists('coordsSp')) {
for(D in unique(round(seq(
  1L, nrow(myParamsBatch), len=min(c(3, nrow(myParamsBatch)))
  )))) {
print(D)
...
maternChol = chol4gpu %*% diag(sqrt(diagGpu))
maternReconstruct = tcrossprod(maternChol)
...
quantile((maternReconstruct - maternCpu)[!lower.tri(maternCpu)], 
         prob=Sprob)
    

cholCpu = as.matrix(geostatsp::matern(
    x=coordsSp,
    param=drop(myParamsBatch[D,1:7]),
    type='cholesky'
    ))

(tcrossprod(cholCpu) - maternCpu)[1:5,1:5]

(cholCpu - maternChol)[1:5,1:5]
...
if(TRUE) {
Nkeep = floor(0.9*ncol(outputChol))
simMat2 = vclMatrix(0, Nkeep*2, ncol(zMatGpu), 
  type = gpuR::typeof(zMatGpu))

gpuBatchMatrix::multiplyLowerDiagonalBatch(
  vclMatrix(outputChol[c(1:Nkeep, 1:Nkeep + ncol(outputChol)), 1:Nkeep]), 
  vclMatrix(diagMat[1:2, 1:Nkeep]), 
  deepcopy(zMatGpu), 
  simMat2, 
  diagIsOne = TRUE, # diagonal of L is one
  transformD = "sqrt", # take the square root of each element of D
  Nglobal = c(32,16,2),#c(32,32,4),#c(64,8,8), # rows, columns, matrix
  Nlocal = c(8,4,1),#c(8,8,1),  # anything, anything 1
  NlocalCache = 2000
  )
} else {
...
gpuBatchMatrix::multiplyLowerDiagonalBatch(
  ...
  diagIsOne = TRUE, # diagonal of L is one
  transformD = "sqrt", # take the square root of each element of D
  Nglobal = c(1,1,1),#c(32,32,4),#c(64,8,8), # rows, columns, matrix
  Nlocal = c(1,1,1),#c(8,8,1),  # anything, anything 1
  NlocalCache = min(c(4000, floor(gpuInfo()$localMem/16)))
  )
}
